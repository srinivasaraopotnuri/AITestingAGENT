    ...
    try {
      if (type === 'plan') {
        ...
        const res = await fetch('/api/generate-test-cases', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ source: 'manual', manualInput: isManual ? manualInput : { title: uploadedFile, description: uploadedText, acceptanceCriteria: '', additionalContext: additionalCtx }, additionalContext: additionalCtx, llmConnection: llmConn })
        })
        const data = await res.json()
        if (!res.ok) throw new Error(data.error || 'Generation failed')
